...
from modules.SpecialFunctions import analytic_phi, F, G, H
...
class MatrixNumerovSolver:
    def __init__(self,
        ...
        self.p_squared = (2 * mass * energy) / hbarc ** 2  # fm ^(-2)
        self.p = self.p_squared ** 0.5  # p = hbar k but hbar here is 1
        ...
        if self.r[0] <= self.epsilon:
            if self.l == 1:
                ...
                self.l_prestore_over_rsq[0] = 0
        self.g_naught = self.l_prestore_over_rsq - self.p_squared
        self.s_prestore = self.phi * (2 * self.mass / hbarc ** 2)
        # for A
        self.one_ten_one = (self.dr ** 2 / 12) * np.array([[1.], [10.], [1.]])
        # for b (s)
        self.coeffs_one_ten_one = (self.dr ** 2 / 12) * diags((1., 10., 1.), (0, -1, -2), 
                                                              shape=(self.n, self.n))
    ###################################################################################################################
    ...
    def const_A(self):
        ...
        if self.use_ab:
            ...
            A_bar[2, -3, 0] = 0.
            A_bar[3, -3, 0] = 1.  # = self.p if using the RHS of the ODE as F and not phi
            A_bar[3, -4, 0] = 1.  # = self.p if using the RHS of the ODE as F and not phi
    ...
    def const_b(self):
        ...
        if self.use_ab:
            ...
            b[-self.n_adjustment:, 0] = self.F[-self.n_adjustment:] * (1 - self.zeta)  # * (1 - self.zeta * self.p) if using the RHS of the ODE as F and not phi
    ...
    def affine_b(self,
        ...
        b[i_start:i_end] = (self.const_b[i_start:i_end, :] @ theta_vec)
        return b
    ...
    def solve(self,
        ...
        """
        A_band = self.affine_A(theta=theta, theta_vec=theta_vec, default_theta=default_theta)[:, self.n_start:]  # trim initial conditions
        b_band = self.affine_b(theta=theta, theta_vec=theta_vec, default_theta=default_theta)[self.n_start:]  # trim initial conditions
        if ret_matrix:
            if self.use_ab:
        ...
        y[1] = self.ypr_0
        if self.use_ab:
            y[self.n_start:] = solve_banded((2, 1), A_band, b_band)
        else:
            y[self.n_start:] = solve_banded((2, 0), A_band, b_band)
        
        return y


class AllAtOnceNumerovSolver:
    def __init__(self,
                 potential,
                 energy=50.,
                 mass=neutron_mass / 2,
                 zeta: bool = 1,
                 y_0=0.,):
        r"""Full Order Model (FOM) solver for the Radial Schrodinger Equation using the All-at-once matrix Numerov method.
        
        This FOM takes advantage of the affine decomposition of the nuclear potential to accelerate 
        computation. Given a potential that has attributes as given in `Potential.py` in this repository, 
        values will be inherited from the passed potential.
        NOTE: This solver will solve for the 'matched' wavefunction. This may cause extremely large values near resonances.
        ...
        self.G = G(self.r, self.p, self.l)
        self.F = F(self.r, self.p, self.l)  # using this in place of phi would be more consistent with the paper. It would also involve throwing factors of p around.
        self.H_plus = H(self.r, self.p, self.l, plus=True, derivative=False)
        self.phi = analytic_phi(self.r, energy=self.energy, l=self.l)
    ...
    def const_A(self):
        ...
        A_bar = np.zeros((self.bandwidth, 
                          self.n + self.n_adjustment, 
                          self.number_of_parameters), dtype=complex)
        ...
        #     B   i  a  =
        A_bar[0, -1, 0] = -self.H_plus[-2]
        A_bar[1, -1, 0] = -self.H_plus[-1]
        A_bar[1, -2, 0] = 0.
        A_bar[2, -2, 0] = 1.  # = self.p if using the RHS of the ODE as F and not phi
        A_bar[2, -3, 0] = 1.  # = self.p if using the RHS of the ODE as F and not phi
    ...
    def const_A_matrix(self):
        ...
        for q in np.arange(self.number_of_parameters):
            A_matrix[:, :, q] = spdiags(self.const_A[:, :, q], (1, 0, -1)).toarray()
        return A_matrix
    ...
    def const_b(self):
        ...
        b[1, 0] += -(1 + 1 * (self.dr ** 2 / 12) * self.const_g[0, 0]) * self.y_0  # this is going to be zero, due to y_0
        
        # set last values
        b[-2:, 0] = self.F[-2:] * (1 - self.zeta)
    ...
    def affine_s(self,
        ...
        This function is defined but never used. Instead `affine_b` is used in its place to handle
        the initial conditions and last elements of y.
        ...
        s[:self.n_slice] = self.const_s[i_start:i_end, :self.n_slice] @ theta_vec
        return s
    ...
    def affine_A(self,
        ...
        if ret_matrix:
            A_matrix = spdiags(A_bar, (1, 0, -1)).toarray()
            return A_matrix
        return A_bar
    ...
    def solve(self,
        ...
        b_band = self.affine_b(theta=theta, theta_vec=theta_vec, default_theta=default_theta)[self.n_start:]  # trim initial conditions
        if ret_matrix:
            matrix = spdiags(A_band, (1, 0, -1)).toarray()
            return matrix, b_band
        elif ret_bar:
            return A_band, b_band
